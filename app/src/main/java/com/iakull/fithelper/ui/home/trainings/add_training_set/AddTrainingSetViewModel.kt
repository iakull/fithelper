...
class AddTrainingSetViewModel(
    ...
    fun start(trainingExerciseId: String, setId: String?, _weight: Int, _reps: Int, _time: Int, _distance: Int) {
    ...
    fun decreaseWeight() {
        weight.value = weight.decrease(5, 0)
    }

    fun increaseWeight() {
        weight.value = weight.increase(5, 10_000)
    }

    fun decreaseReps() {
        reps.value = reps.decrease(1, 0)
    }

    fun increaseReps() {
        reps.value = reps.increase(1, 1_000)
    }

    fun decreaseTime() {
        time.value = time.decrease(15, 0)
    }

    fun increaseTime() {
        time.value = time.increase(15, 24 * 60 * 60)
    }

    fun decreaseDistance() {
        distance.value = distance.decrease(100, 0)
    }

    fun increaseDistance() {
        distance.value = distance.decrease(100, 100_000)
    }

    private fun LiveData<Int>.decrease(subtrahend: Int, minValue: Int) = (value!! - subtrahend).coerceAtLeast(minValue)

    private fun LiveData<Int>.increase(term: Int, maxValue: Int) = (value!! + term).coerceAtMost(maxValue)
}